<script lang="ts">
  ...
  function getInputColor(s: SOLVE) {
    ...
    if (t === "" || /^([1-9]\d*|DNF|DNS)$/.test(t)) {
      s.penaltyType =
        t === "DNS" ? PENALTY.DNS : t === "DNF" ? PENALTY.DNF : PENALTY.NONE;
      return "base";
    }
    ...
    return contest.contestants.filter(
      (c) =>
        ...
        !roundGroup.some((r) =>
          r.some(
            (rnd) =>
              rnd.category.id === catId &&
              rnd.round === roundNum &&
              c.user.id === rnd.contestant.id
          )
        )
    );
  }
    ...
    <Tabs style="underline" contentClass="dark:bg-gray-800 mt-4">
      <TabItem open>
        ...
        <div class="grid gap-4 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2">
          ...
          <div>
            <Label for="inscription-start" class="mb-2"
              >Inscripción (inicio)</Label
            >
            <Input
            ...
            <div class="w-full flex flex-wrap gap-2">
              {#each categories as ct}
                <WcaCategory
                  ...
                  selected={!!contest.categories.find(
                    (cat) => cat.category.id === ct.id
                  )}
                />
          ...
          <!-- Visible -->
          <div class="flex items-center gap-2">
            <Label for="visible" class="cursor-pointer select-none"
              >Visible</Label
            >
            ...
            <div class="w-full flex flex-wrap gap-4 justify-center">
              {#each contest.categories as ct}
                <div class="grid place-items-center">
                  ...
                  <div
                    class="flex items-center justify-between border gap-2 rounded-md"
                  >
                    <button
                      class="p-1"
                      type="button"
                      on:click={() => (ct.rounds = Math.max(1, ct.rounds - 1))}
                      >-</button
                    >
                    <Span>{ct.rounds}</Span>
                    <button
                      class="p-1"
                      type="button"
                      on:click={() => (ct.rounds = Math.min(5, ct.rounds + 1))}
                      >+</button
                    >
                  </div>
                </div>
            ...
            <TableBody>
              {#each contest.contestants as c (c.user.username)}
                <TableBodyRow>
                  ...
                  <TableBodyCell>
                    <Button class="p-2" on:click={() => editUser(c)}
                      ><EditIcon /></Button
                    >
                    <Tooltip>Editar categorías</Tooltip>
                    <Button
                      class="p-2"
                      on:click={() => deleteContestant(c)}
                      color="red"><DeleteIcon /></Button
                    >
...
<!-- EDIT USER DIALOG -->
<Modal
  title={`Categorías de ${selectedUser?.user?.name}`}
...
>
  <div class="w-full flex flex-wrap items-center justify-center gap-2">
    {#each contest.categories as ct}
      <WcaCategory
        class="cursor-pointer"
        on:click={() => toggleSelectUser(ct.category)}
        icon={ct.category.scrambler}
        selected={!!selectedUser?.categories?.find(
          (cat) => cat.id === ct.category.id
        )}
      />
      <Tooltip>{ct.category.name}</Tooltip>
    {/each}
  ...
  <svelte:fragment slot="footer">
    <div class="flex mx-auto justify-center gap-4">
      <Button on:click={() => (editDialog = false)} color="alternative"
        >Cancelar</Button
      >
...
<Modal title="Agregar resultado" bind:open={addResult}>
  <div class="flex items-center justify-center gap-2">
    <Button color="alternative" class="gap-2">
      <WcaCategory size="1rem" icon={round.category.scrambler} />
      {round.category.name}
    </Button>
      ...
      {#each contest.categories as ct}
        ...
        >
          <div class="flex gap-2 items-center">
            <WcaCategory size="1rem" icon={ct.category.scrambler} />
            {ct.category.name}
          </div>
        </DropdownItem>
      {/each}
    ...
    {#if getContestants(round.category.id, round.round).length > 0}
      ...
      <Dropdown placement="bottom" bind:open={dropdownUsers}>
        {#each getContestants(round.category.id, round.round) as cnt}
          <DropdownItem
            on:click={() => {
              dropdownUsers = false;
  ...
  <svelte:fragment slot="footer">
    <div class="flex mx-auto justify-center gap-4">
      <Button on:click={() => (addResult = false)} color="alternative"
        >Cancelar</Button
      >
      <Button disabled={!validateRound(round)} on:click={saveResult}
        >Aceptar</Button
      >
    </div>
  </svelte:fragment>
...
<Modal bind:open={deleteUserDialog} title="Eliminar competidor" size="xs">
  <Heading tag="h4" class="text-center"
    >¿Desea eliminar a {deleteUserData?.user.name}?</Heading
  >

  <div class="flex mx-auto justify-center gap-4">
    <Button on:click={() => (deleteUserDialog = false)} color="alternative"
      >Cancelar</Button
    >
...
>
  <div class="flex flex-col items-center justify-center">
    ...
    <Heading tag="h4" class="text-center"
      >¿Desea eliminar la competencia "{contest.name}"?</Heading
    >
