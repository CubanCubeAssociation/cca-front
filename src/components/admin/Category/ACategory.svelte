...
<Card class="mt-4 max-w-sm w-[calc(100%-2rem)] mx-auto mb-8">
  <Heading class="text-3xl text-center">{ type === 'update' ? `Editar "${ category.name }"` : 'Crear categoría'}</Heading>
  ...
  <form class="mt-8 grid gap-2" on:submit|preventDefault={ save }>
    ...
    <div class="col-span-full flex flex-wrap gap-2 justify-center">
      ...
      <Button class="text-white" type="submit">
        <SaveIcon size="1.2rem"/> <Span class="ml-1">{ type === 'update' ? 'Guardar' : 'Crear' }</Span>
      </Button>
    </div>
  </form>
</Card>
