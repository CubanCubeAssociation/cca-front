...
<main class={`select-container relative cursor-pointer border
  border-solid border-gray-400 h-max-10 bg-white
  rounded-md ` + (cl || '')}>
  <button tabindex="0"
    on:click|self|stopPropagation={handleClick}
    class="p-2 flex items-center rounded-md select-none pr-8
  ...
  <div class="options bg-white
    p-2 w-full rounded-md border border-solid border-gray-400
    ...
    {#each items as item, pos}
      ...
      {/if}
      <button
        class="option text-black text-left transition-all duration-200 hover:bg-gray-200 p-1 rounded-md"
