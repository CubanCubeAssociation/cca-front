...
function inverse(type: PuzzleType, sequence: string): string {
  let arr: string[] = [];
  let res = [];
    ...
    for (let i = arr.length - 1; i >= 0; i -= 1) {
      ...
      if (/^([LRDlrd](\+|-){1,2})$/.test(mv)) {
        ...
        if (/^([ULFRBDy]\d*'?)$/.test(mv)) {
          ...
          res.push(
            `[${mv[1]}${turns === 1 || turns === -1 ? "" : Math.abs(turns)}${turns < 0 ? "" : "'"}]`
          );
        }
      }
    }
...
class Solver {
  ...
  invert(seq: string[]): string[] {
    return seq.map(s => inverse(this.tokenizerType, s));
  }
  ...
  invertFlat(seq: IToken[]): IToken[] {
    ...
    for (let i = seq.length - 1; i >= 0; i -= 1) {
      if (seq[i].type === "Move") {
        ...
        cp.value = inverse(this.tokenizerType, cp.value);
        res.push(cp);
      } else {
        res.push(seq[i]);
      }
