...
import { bezier, circle, mod } from "@helpers/math";
...
export function roundStickerCorners(
  ...
  const RAD = typeof rd === "function" ? 0.11 : rd || 0.11;
  ...
  for (let i = 0, maxi = pts.length; i < maxi; i += 1) {
    ...
    if (abs1 < ROUND_THRESHOLD && abs2 < ROUND_THRESHOLD) {
      ...
      if (calcPath) (newSt as BezierSticker).parts.push(pts[i].clone());
      continue;
    }
    ...
    if (isCircle) {
      circle(pts[i].add(v1), pts[i], pts[i].add(v2), PPC).forEach(p => {
        newSt.points.push(p);
        if (calcPath) (newSt as BezierSticker).parts.push(p.clone());
      });
    } else {
