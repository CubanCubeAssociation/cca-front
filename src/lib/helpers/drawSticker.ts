...
import type { PuzzleInterface, PuzzleType } from "@interfaces";
...
export function drawStickers(
  ...
  for (let i = 0, maxi = allStickers.length; i < maxi; i += 1) {
    ...
    ctx.fillStyle = new Color(pInterf.palette[st.color]).toHex();
